...
class ProfileVC: UIViewController {
  ...
  // 현재 로그인 사용자 유져
  var currnetUserData: User? {
    didSet {
      guard let user = currnetUserData else { return }
      ...
      rankingValue = userDatas.firstIndex(of: currnetUserData!) ?? 0
      self.menuCollection.reloadData()
    }
  }
  
  var rankingValue: Int = 0 {
    didSet {
      self.imageLabel1.text = "RANK: \(rankingValue + 1) st"
    }
  }
...
extension ProfileVC: UICollectionViewDataSource {
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    let item = collectionView.dequeueReusableCell(withReuseIdentifier: ProfileViewCustomCell.identifier,for: indexPath) as! ProfileViewCustomCell
